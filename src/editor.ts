...
import param, { BiomeConfig, ElevationConfig, RenderConfig, RiverConfig } from "./config";
import {makeMesh} from "./mesh";
import Painting from "./map-gen-4/painting";
import Renderer from "./render";
import type {Mesh} from "./map-gen-4/types.d";

import MapWorker from './map-gen-4/worker?worker'

type InitialParamDataEntry<T> = [T,number,number,number];
interface InitialParamData {
  elevation: InitialParamDataEntry<keyof ElevationConfig>[];
  biomes: InitialParamDataEntry<keyof BiomeConfig>[];
  rivers: InitialParamDataEntry<keyof RiverConfig>[];
  render: InitialParamDataEntry<keyof RenderConfig>[];
}

const initialParams: InitialParamData = {
    elevation: [
        ['seed', 187, 1, 1 << 30],
        ['island', 0.5, 0, 1],
...
type InitialParams = keyof typeof initialParams & keyof typeof param;
...
function main({mesh, t_peaks}: { mesh: Mesh; t_peaks: number[]; }) {
    ...
    for (let phase of (['elevation', 'biomes', 'rivers', 'render'] as InitialParams[])) {
        ...
        for (let [name, initialValue, min, max] of initialParams[phase]) {
            ...
            param[phase][name] = param[phase][name] || initialValue;
    ...
    function redraw() {
        ...
        window.dispatchEvent(new CustomEvent('on-map-update', { detail: render.screenshotCanvas }));
    }
    ...
    worker.addEventListener('messageerror', (event: {data: any}) => {
        console.log("WORKER ERROR", event);
    });
    
    worker.addEventListener('message', (event: {data: any})  => {
        working = false;
